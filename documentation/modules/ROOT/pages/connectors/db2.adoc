...
Unless overridden via the link-prefix}:{link-db2-connector}#db2-property-transaction-topic-prefix[`transaction.topic.prefix`] option,
...
[[db2-property-column-mask-hash-v2]]<<db2-property-column-mask-hash-v2, `column.mask.hash.v2._hashAlgorithm_.with.salt._salt_`>>
|_n/a_
...
|[[db2-property-transaction-topic-prefix]]<<db2-property-transaction-topic-prefix, `transaction.topic.prefix`>>
|`database.server.name`
|Controls the name of the topic to which the connector sends transaction metadata messages. The topic name has this pattern: _transaction.topic.prefix.transaction_. If not configured, it defaults to _database.server.name.transaction_.
