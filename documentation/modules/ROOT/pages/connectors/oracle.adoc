...
Unless overridden via the link-prefix}:{link-oracle-connector}#oracle-property-transaction-topic-prefix[`transaction.topic.prefix`] option,
...
Each identifier is of the form `__<schema_name>.<table_name>__`. +
 +
...
|[[oracle-property-transaction-topic-prefix]]<<oracle-property-transaction-topic-prefix, `transaction.topic.prefix`>>
